...
export default function WheelPage () {
  ...
  return (
    <section class="flex flex-col items-center p-8">
      ...
      <div
        class="w-96 h-96"
        style={`border-radius: 50%; background: ${conicGradient(options())}`}
        />
        <Index each={options()} >
          {(option, i) => (
            <div 
              style={`transform: rotate(${labelRotation(options().length)(i)}deg); top: 172px`}
              class="flex absolute w-96"
            >
            <label 
              class="p-2 w-96 text-right"
            >
              {option}     
            </label>
        ...
        <IconAntDesignCaretLeftFilled class="absolute" style="top: 11.4rem; right: -1rem;" />
      </div>
      <div
        class="mt-11"
      >
        ...
        <div class="flex flex-col">
          ...
          <Link href={`/${search()}`} >
          <button 
            class="mt-4 inline-block min-w-52 px-6 py-2.5 bg-black text-white hover:shadow-lg font-medium text-xs leading-tight uppercase rounded focus:bg-gray-100 focus:ring-0 active:bg-gray-100 transition duration-150 ease-in-out"
