...
export default function Input() {
  ...
  const hasDuplicates = createMemo(() =>
    Object.values(errors()).find((duplicate) => duplicate > 0)
  );
  const blankOptions = createMemo(() =>
  ...
  return (
    <section class="flex flex-col items-center p-8">
      <div class="pl-4">
          ...
          <div class="mt-2">
            <Show when={options().length < 10}>
              <button
                ...
                onClick={() =>
                  setParams({
                    options: [...options(), `Option ${count() + 1}`],
                  })
                }
              >
                Add Option
        ...
        <p class="min-h-6 w-52 text-center text-red-700 ">{errorMessage()}</p>
        <div class="mt-4">
          <Link href={`/wheel${search()}`}>
            <button
